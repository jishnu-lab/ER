...
KfoldCV_tenary = function(k, Y, X, X_high_var, methods, pt_option = "perm_X", 
                          Y_type = "factor", delta_grid = NULL, standardize = F,
                          ER_sparse = F) {
  ...
  if (pt_option == "perm_X")
    perm_col_ind <- sample(1:ncol(X_high_var))
  else if (pt_option == "perm_Y_before_split")
    perm_row_ind <- sample(1:nrow(X_high_var))
  ...
  for (i in 1:k) {
    ...
    trainX_X_high_var <- X_high_var[-valid_ind,]
    validX_X_high_var <- X_high_var[valid_ind,,drop = F]
    
    for (j in 1:length(methods)) {
      method_j <- methods[j]
      if (method_j == "Perm-Lasso") {
        if (pt_option == "perm_X") {
          perm_train_X <- trainX_X_high_var[,perm_col_ind]
          res_ij <- calAcc(validY, validX_X_high_var, perm_train_X, trainY, method_j, standardize,
                           trainY_label, Y_type)
        } else if (pt_option == "perm_X_Y") {
          perm_train_X <- trainX_X_high_var[,perm_col_ind]
          ...
          res_ij <- calAcc(validY, validX_X_high_var, perm_train_X, perm_train_Y, method_j, standardize,
          ...
          res_ij <- calAcc(validY, validX_X_high_var, trainX_X_high_var, perm_train_Y, method_j, standardize,
                           perm_train_Y_label, Y_type)
        }
      } else if (method_j == "ER-Lasso-LS" | method_j == "ER-Lasso-Dantzig" |
                 method_j == "Lasso") {
        
        res_ij <- calAcc(validY, validX_X_high_var, trainX_X_high_var, trainY, method_j, standardize, 
...
calAcc = function(validY, validX, trainX, trainY, method, standardize, 
  ...
  if (method == "Lasso" || method == "Perm-Lasso") {
    ...
    index_lbd_min <- which(cvfit$lambda == cvfit$lambda.min)
    index_lbd = min(length(cvfit$lambda), index_lbd_min + 3)
    # index_lbd = max(1, index_lbd_min - 0)    # for turbo data only
    
    beta_hat <- coef(cvfit, s = cvfit$lambda[index_lbd])[-1]
    supp_beta_hat <- which(beta_hat != 0)
    
    if (length(supp_beta_hat) == 0) {
      cat("Lasso selects no variable.\n")
      unit_cor <- cov(trainX, trainY)
      supp_beta_hat <- order(abs(unit_cor), decreasing = T)[1:5]  # if lasso selects no variable, pick the most correlated 5 features instead
    }
    ...
    supp_beta_hat <- which(beta_hat != 0)
    if (length(supp_beta_hat) == 0) {
      cat("Composite selects no variable.\n")
      unit_cor <- cov(trainX, trainY)
      supp_beta_hat <- order(abs(unit_cor), decreasing = T)[1:5]  # if lasso selects no variable, pick the most correlated 5 features instead
    }
