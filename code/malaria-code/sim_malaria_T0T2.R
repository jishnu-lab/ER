...
sub_data <- subset(raw_data, raw_data$time_point %in% c("T0", "T2"))
sub_data$time_point <- factor(sub_data$time_point, levels = c("T0", "T2"),
                              labels = c(0, 1))
...
# write.table(CV_result, file = "CV_result_T0T2.txt", row.names = F, col.names = T)


CV_result = read.table("CV_result_T0T2.txt", header = T)
