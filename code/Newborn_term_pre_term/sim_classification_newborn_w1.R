...
# 
# 
# save_LOVE_result(list_clusters, delta, lbd, F, "newborn_dataset/clusters_week1")
# save_LOVE_result(list_clusters_pure, delta, lbd, T, "newborn_dataset/clusters_week1")
...
# file_name <- "../output/newborn_dataset/week1_var_selection.txt"
...
Plot_2D(Y_label, Z_tilde[,Z_ind], c("Pre-term", "Term"), axis_names = c("ER1", "ER2"))
...
# write.table(pred_val, file = "../output/newborn_dataset/week1_sparse.txt", row.names = F, col.names = T)
write.table(pred_val, file = "../output/newborn_dataset/week1_non_sparse.txt", row.names = F, col.names = T)

# pred_val <- read.table("../output/newborn_dataset/week1_sparse.txt", header = T)
pred_val <- read.table("../output/newborn_dataset/week1_non_sparse.txt", header = T)
...
# 0.977      0.963      0.725      0.681      0.970      0.525 

## sparse 
# ER         PFR        PLS        Lasso      Perm-Lasso 
# 0.855      0.725      0.755      0.968      0.529 
...
# write.table(CV_result, file = "../output/newborn_dataset/week1_accuracy_04_11.txt", row.names = F, col.names = T)
# write.table(CV_result, file = "../output/newborn_dataset/week1_accuracy_sparse.txt", row.names = F, col.names = T)

CV_result <- read.table("../output/newborn_dataset/week1_accuracy_04_11.txt", header = T)
# CV_result <- read.table("../output/newborn_dataset/week1_accuracy_sparse.txt", header = T)

methods[2] = "Composite"


Violin_plot(methods, CV_result, MSE_flag = F, label_names = methods[c(1,2,5,3,4,6)])
